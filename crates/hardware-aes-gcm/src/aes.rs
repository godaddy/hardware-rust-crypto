...
impl Aes256 {
    ...
    /// Encrypts eight independent blocks with interleaved round chains.
    pub(crate) fn encrypt_blocks8(&self, blocks: &mut [[u8; 16]; PAR_BLOCKS]) {
...
mod imp {
    ...
    impl Aes256 {
        ...
        pub(super) fn encrypt_blocks8(&self, blocks: &mut [[u8; 16]; super::PAR_BLOCKS]) {
            // SAFETY: Aes256 can only be constructed after hardware_available
            // has checked the target features required by the inner method.
            unsafe { self.encrypt_blocks8_inner(blocks) };
        }
        ...
        /// Eight independent AESE/AESMC chains, advanced one round at a time
        /// so the round instructions pipeline across blocks.
        #[target_feature(enable = "aes", enable = "neon")]
        unsafe fn encrypt_blocks8_inner(&self, blocks: &mut [[u8; 16]; super::PAR_BLOCKS]) {
            let mut state = [vdupq_n_u8(0); super::PAR_BLOCKS];
...
mod imp {
    ...
    impl Aes256 {
        ...
        /// Eight independent AESENC chains, advanced one round at a time so
        /// the round instructions pipeline across blocks.
        #[target_feature(enable = "sse2", enable = "aes")]
        unsafe fn encrypt_blocks8_inner(&self, blocks: &mut [[u8; 16]; super::PAR_BLOCKS]) {
            let mut state = [_mm_setzero_si128(); super::PAR_BLOCKS];
            for (lane, block) in state.iter_mut().zip(blocks.iter()) {
...
mod imp {
    ...
    impl Aes256 {
        ...
        pub(super) fn encrypt_blocks8(&self, _blocks: &mut [[u8; 16]; super::PAR_BLOCKS]) {
            match *self {}
        }
    }
