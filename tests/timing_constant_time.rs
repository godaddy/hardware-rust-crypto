...
//!
//! Note what is deliberately NOT asserted: valid and invalid tags take
//! *different* times, because a valid tag runs the CTR decryption pass and an
//! invalid tag returns early without it. That difference reveals only the
//! authentication outcome, which is the public `Result` the caller already
//! receives - it is the intended verify-before-decrypt behavior, not a
//! channel. What must not leak is *how much* of the tag matched, which the
//! mismatch-position test covers.
//!
...
fn tag_comparison_independent_of_mismatch_position() {
    ...
    // tag mismatches. A constant-time comparison cannot tell them apart, so
    // both also skip the CTR pass identically - the only timing variable is
