...
cargo asm -p hardware-aes-gcm --lib 'hardware_aes_gcm::KeyState::init_in_place' > init.s
...
What is deliberately **not** asserted: that valid and invalid tags take the
same time. They do not - a valid tag runs the CTR decryption pass, an invalid
tag returns early without it. That difference reveals only the authentication
outcome, which is the public `Result`; it is the intended verify-before-
decrypt behavior, not a channel.
