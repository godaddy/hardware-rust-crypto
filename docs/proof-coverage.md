...
| --- | --- | --- |
| **Extraction-based proof of the AES-*calling* composition glue** (`seal`/`open`, SIV derivation) | These call the intrinsic backends, so CBMC/Kani can't reach them; `prove_composition.py` proves a faithful *model* (T2), not the compiled source. | hax/F\* or SAW. Attempted; blocked on hax's pinned-nightly driver + no F\* engine. Reproducible plan in [`proofs/hax/README.md`](../proofs/hax/README.md). |
