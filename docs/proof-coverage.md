...
| `increment_counter` == SP 800-38D inc_32 — **proven over Rust MIR** | T1 | **crux-mir** (Galois) symbolically executes the MIR — a fourth independent toolchain; sidesteps the LLVM poison that limited SAW | `proofs/crux-mir/increment_counter.rs` |
