...
| **Location** | `src/random/mod.rs` - `reseed` (L386), `reseed_blend` (L422); `src/random/entropy.rs` |
...
| **Status** | Open |
| **Location** | `src/random/entropy.rs` - `stuck_output` (L62), `cpu_rng_fill` (L32) |
...
| **Status** | Open (residual) |
| **Location** | `src/aes_gcm/{aes,ghash,lib}.rs`; `src/random/{aes_ctr,entropy,lib}.rs` |
...
| **Status** | Open (residual) |
| **Location** | `src/aes_gcm/mod.rs` - `constant_time_eq` (L887); `src/aes_gcm/ghash.rs` - `mulx` (L201) |
...
|t| ~ 0.2, two orders of magnitude below the |t| ~ 267 an early-exit
comparison produces.
...
| **Location** | `src/aes_gcm/ghash.rs` (`mulx` `u128`); SIMD by-value locals across both crates |
...
The GCM composition is implemented in `src/aes_gcm/mod.rs` (`KeyState::seal`
and `KeyState::open`) over the AES and GHASH backends.
...
J0 mask into the GHASH output. Decrypt writes plaintext into caller-controlled
output while authenticating, then compares the tag in constant time. If the
comparison fails, the plaintext-length prefix of the caller buffer is zeroized
before `Error::Decrypt` is returned.
...
```rust
// src/aes_gcm/ghash.rs (init_in_place)
...
**Stitched bulk AEAD.** On the bulk encrypt and decrypt paths, the keystream
batch and GHASH of the previous ciphertext batch are emitted as two independent
instruction sequences in one `#[target_feature]` body (`seal_bulk_inner` /
`open_bulk_inner`), software-pipelined so the AES and carryless-multiply
pipelines overlap. The instructions are the data-independent AESE/AESMC
(AESENC/AESENCLAST), PMULL/PMULL2 (PCLMULQDQ), XOR, and in-register byte-reverse
(REV / PSHUFB), the loop bound is the public batch count, and no memory index
depends on a secret, so the constant-time argument of section 6 holds. Decrypt
writes plaintext before the final tag comparison; public wrappers zeroize the
written range on authentication failure.
...
- *tag comparison vs. mismatch position*: |t| = 0.178 (means 437.6 ns /
  437.6 ns) - no early-exit leak.
- *decrypt time vs. ciphertext content* (symmetric pools): |t| = 1.537.
...
| --- | ---: | ---: | ---: |
| `src/aes_gcm/aes.rs` | 35 | 9 | 35 |
| `src/aes_gcm/ghash.rs` | 54 | 28 | 54 |
| `src/aes_gcm/mod.rs` | 11 | 1 | 13 |
| `src/aes_gcm/fork.rs` | 1 | 0 | 1 |
| `src/aes_gcm/nonce.rs` | 0 | 0 | 0 |
| `src/random/aes_ctr.rs` | 18 | 7 | 17 |
| `src/random/mod.rs` | 3 | 0 | 3 |
| `src/random/entropy.rs` | 3 | 2 | 3 |
| `src/random/fork.rs` | 1 | 0 | 1 |
...
| `aes::tests` (unit) | 2 | software key schedule (cfg(miri) path) == hardware round keys; and `AES_SBOX` == FIPS-197 `affine(inverse_GF(2^8)(x))` for all 256 inputs (and a bijection) |
...
cargo test --release -p hardware-rust-crypto
...
```
tag-mismatch-position:        |t| = 0.178   (mean0 437.6ns, mean1 437.6ns)
low-vs-high-entropy-content:  |t| = 1.537   (mean0 168.9ns, mean1 168.7ns)
```
