...
| **Model checking (compiled Rust)** | Kani/CBMC verifies the **actual compiled** intrinsic-free logic over all inputs: GCM/SIV counter increments == the spec increments, J0 layout, length validation, the nonce parser, the two envelope splitters (no panic, correct boundaries), and `constant_time_eq` == bytewise equality on equal-length tags (the auth decision never accepts a wrong tag or rejects a right one) | `cargo kani` (`cfg(kani)` harnesses) |
...
- **hax / Cryspen** - extract the safe Rust glue to F\*/Coq and prove the AEAD
  ...
  *Attempted:* the `cargo-hax` frontend installs and the F\* backend is present,
  but extraction is blocked on hax's pinned-nightly rustc driver and the absence
  of an F\* engine to check the output. The attempt, what it would add, and the
  exact steps to finish it are recorded in [`proofs/hax/README.md`](../proofs/hax/README.md).
